...
class JudgeJob < ActiveJob::Base
	...
	def perform(code, time, space)
		...
		factor = Slows.include?(lang) ? 1 : 2
		if factor == 2 || compile?(code_id, lang)
			time_limit = time
			space_limit = space
			exec_cmd =  EXEC_CMD[lang]
			run_many(problem_id, exec_cmd, time_limit * factor, space_limit * factor) 
		end
		Status.create({
		                run_id: @code.id,
		...
		if @result ==  AC 
			...
			unless user_detail.ac_records.include?(code.problem_id)
				user_detail.ac_records << code.problem_id
	...
	def run_many(problem_id, exec_cmd, time_limit, space_limit)
		testdatas = JSON.parse File.read("#{TEST_PATH}/#{problem_id}.json")
		testdatas.each do |testdata|
			stdin = testdata[0]
			time_before = Time.now
			result = Open3.popen3(exe_cmd, rlimit_cpu: time_limit, rlimit_rss: space_limit) do |i,o,e,wt|
				i.puts(stdin) 
				if wt.value.signaled?
					TE
				elsif e.read.present?
					RE
